...
def main():
    ...
    average, median, highest_marks, highest_ID, marks_range = generate_stats(df, MARKS_COLUMN, ID_COLUMN)

    # Display general statistics
    table = PrettyTable(["Average", "Median", "Range", "Highest Marks", "Topper"] )
    table.add_row([round(average, 2), round(median, 2), round(marks_range, 1),round(highest_marks, 1), highest_ID])
    print(table)

    # Prompt for additional statistics
    print("Display additional statistics? Enter 1 for yes and 0 for no: ", end="", flush=True)
    try :
    ...
    if (display_additional == 1):
        ...
        add_table = PrettyTable(["90th percentile", "Interquartile Range", "Standard deviation"])
    ...
    try:
        ...
        print(f"{bcolors.BOLD}{bcolors.OKGREEN} Histogram created for {SPREADSHEET} in 'img/'  {bcolors.ENDC}")
    except Exception as _: 
        print(f"{bcolors.BOLD}{bcolors.FAIL} Histogram generation failed {bcolors.ENDC}")
